...
ASTRAL_data <- data[data$ASTRAL_score != 2, ]
ASTRALglm <- glm(ASTRAL_score~
                   no_of_taxa+
...
# making the ASTRALGLM table
glm_summary <- summary(ASTRALglm)
...
kable_table <- kable(glm_summary_with_stars, format = "html", align = "c", caption = "GLM on Phylogenetic Accuracy of Reconstructed ASTRAL Trees (MR-High)") %>%
  kable_styling(
...
save_path_png <- "12_high/results/big_scores/astralGLM.png"
webshot(temp_html, file = save_path_png, zoom = 2)
unlink(temp_html)
# R^2 analysis

# Rename the parameters in the ASTRAL_data dataset
colnames(ASTRAL_data) <- c("Taxa", "CellsPerTumour", "EffPopSize", "MetastaticRate",
...
ASTRAL_r2 <- regsubsets(ASTRAL_score ~ Taxa + CellsPerTumour +
                          EffPopSize + MetastaticRate + MutationRate + NumberOfSites +
                          ExpGrowthRate + ADO + AmpError + SeqError,
                        data = ASTRAL_data)

reg.summary=summary(ASTRAL_r2)
plot(ASTRAL_r2, scale = "r2", main = "Subset Selection - Phylogenetic Accuracy of Reconstructed ASTRAL Trees (MR-High)")
