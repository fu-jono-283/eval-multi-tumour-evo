...
cellphy.sh RAXML --msa "$no_outgcell_consensus_file" --model GT16+FO --seed 2 --threads $cpus_per_task --force perf_threads --prefix "$vcf_output_prefix" --force model_lh_impr #--blopt nr_safe --pat-comp off
}
...
mv "logs/3_cellphy_log/vcf/score_logs/${SLURM_JOB_ID}_vcf.out" "logs/3_cellphy_log/vcf/score_logs/$iteration_identifier_${SLURM_JOB_ID}_vcf.out"
