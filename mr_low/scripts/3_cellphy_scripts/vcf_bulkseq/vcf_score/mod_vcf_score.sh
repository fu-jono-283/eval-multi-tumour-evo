...
process_vcf_folder() {
local iteration_identifier="_pop87428_sites95171r2"
...
cellphy.sh RAXML --msa "$no_outgcell_consensus_file" --model GT16+FO --seed 2 --threads 2 --force perf_threads --prefix "$vcf_output_prefix" --force model_lh_impr #--blopt nr_safe --pat-comp off
}
...
mv "data/3_cellphy_log/vcf/score_logs/${SLURM_JOB_ID}_vcf.out" "data/3_cellphy_log/vcf/score_logs/_pop87428_sites95171r2_${SLURM_JOB_ID}_vcf.out"
