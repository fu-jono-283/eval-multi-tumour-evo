...
process_single_folder() {
    local iteration_identifier="_pop54701_sites86934r7"
    local iteration_folder="$single_ccoal_output_folder/_pop54701_sites86934r7"
    ...
    cellphy.sh RAXML --msa "$haplotype_file" --model GT16+FO+E --seed 2 --threads 8 --force perf_threads --prefix "$single_output_prefix" 
}

execute_r_script() {
    local iteration_identifier="_pop54701_sites86934r7"
    local iteration_folder="$single_ccoal_output_folder/_pop54701_sites86934r7"
    local gene_tree_file=$(find "$simphy_folder/_pop54701_sites86934r7" -type f -name 'g_trees*.trees' -print -quit)
...
mv "data/3_cellphy_log/single/${SLURM_JOB_ID}_single.out" "data/3_cellphy_log/single/_pop54701_sites86934r7_${SLURM_JOB_ID}_single.out"
