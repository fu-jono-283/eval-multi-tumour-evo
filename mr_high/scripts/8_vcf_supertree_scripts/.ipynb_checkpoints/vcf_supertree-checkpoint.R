...
destination_directory <- "data/8_vcf_super"
...
for (replicate_dir in replicate_directories) {
  ...
  tree_files <- list.files(replicate_dir, pattern = "vcf.raxml.bestTree", full.names = TRUE)
...
python_script <- "scripts/8_vcf_supertree_scripts/merge_trees.py"
system2("python", args = python_script)

iteration_folders <- list.dirs("data/8_vcf_super", full.names = TRUE, recursive = FALSE)

for (iteration_folder in iteration_folders) {
 if (iteration_folder != "data/8_vcf_super" && !grepl("/\\.ipynb_checkpoints$", iteration_folder)) {
