...
# vcf_FastRFSGLM analysis
data$vcf_FastRFS_score <- 1 - data$vcf_FastRFS_score
vcf_FastRFS_data <- data[data$vcf_FastRFS_score != 2, ]
vcf_FastRFSglm <- glm(vcf_FastRFS_score~
                   no_of_taxa+
                   ...
                   amplification_error+
                   sequencing_errors, 
                 data = vcf_FastRFS_data)
summary(vcf_FastRFSglm)




# making the vcf_FastRFSGLM table
glm_summary <- summary(vcf_FastRFSglm)
...
kable_table <- kable(glm_summary_with_stars, format = "html", align = "c", caption = "GLM on Phylogenetic Accuracy of FastRFS on VCF-PB Trees (MR-Full)") %>%
  kable_styling(
...
save_path_png <- "12/results/big_scores/vcf_fastrfsGLM.png"
webshot(temp_html, file = save_path_png, zoom = 2)
unlink(temp_html)
# R^2 analysis

# Rename the parameters in the vcf_FastRFS_data dataset
colnames(vcf_FastRFS_data) <- c("Taxa", "CellsPerTumour", "EffPopSize", "MetastaticRate",
...
vcf_FastRFS_r2 <- regsubsets(vcf_FastRFS_score ~ Taxa + CellsPerTumour +
                          EffPopSize + MetastaticRate + MutationRate + NumberOfSites +
                          ExpGrowthRate + ADO + AmpError + SeqError,
                        data = vcf_FastRFS_data)

reg.summary=summary(vcf_FastRFS_r2)

png("r2_plot.png", width = 10, height = 6, units = 'in', res = 300)
plot(vcf_FastRFS_r2, scale = "r2", main = "Subset Selection - Phylogenetic Accuracy of FastRFS on VCF-PB Trees (MR-Full)")
