...
kable_table <- kable(glm_summary_with_stars, format = "html", align = "c", caption = "GLM on Phylogenetic Accuracy of Reconstructed H-CS Trees (MR-Full)", escape = FALSE) %>%
  kable_styling(
...
save_path_png <- "12/results/small_scores/hapconGLM.png"
...
plot(hapcon_r2, scale = "r2", main = "Subset Selection - Phylogenetic Accuracy of Reconstructed H-CS Trees (MR-Full)")
dev.off()

library(dplyr)


glm_summary_with_stars[] <- lapply(glm_summary_with_stars, function(x) gsub("\"", "", x))
colnames(glm_summary_with_stars) <- make.names(colnames(glm_summary_with_stars))

glm_summary_with_stars <- glm_summary_with_stars %>%
  mutate(across(where(is.numeric), function(x) format(as.numeric(x), scientific = TRUE, digits = 3)))

write.csv(glm_summary_with_stars, "hapconGLM_summary_12.csv", row.names = TRUE, quote = FALSE)
