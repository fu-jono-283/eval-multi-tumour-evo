...
for iteration_folder in "$source_dir1"/*; do
  if [ -d "$iteration_folder" ]; then
    ...
    cp -f "$corresponding_iteration_folder/log" "$destination_iteration_folder/logs/"
      ...
      simphy_conf_file="$iteration_folder/"$iteration_folder".conf"
      if [ -f "$simphy_conf_file" ]; then
        cp -f "$simphy_conf_file" "$destination_iteration_folder/parameters/"$iteration_folder".conf"
      fi

      simphy_params_file="$iteration_folder/$(basename "$iteration_folder").params"
      if [ -f "$simphy_params_file" ]; then
        cp -f "$simphy_params_file" "$destination_iteration_folder/parameters/simphy_params"
      fi
