...
for (iteration_folder in iteration_folders) {
  ...
  for (cutoff in cutoff_values) {
    ...
    if (file.exists(consensus_file)) {
      ...
      row <- data.frame(Iteration = iteration_number, CutoffValue = cutoff, N_count = total_N_bases)
      combined_data <- rbind(combined_data, row)
    }
  }
}

existing_data <- read.csv("dataframes/combined_data_120.csv")
...
mega_data <- data.frame(Proportion_N = mega_data$Proportion_N, mega_data[, setdiff(names(mega_data), "Proportion_N")])

write.csv(mega_data, file = "dataframes/ncountdata.csv", row.names = FALSE)
