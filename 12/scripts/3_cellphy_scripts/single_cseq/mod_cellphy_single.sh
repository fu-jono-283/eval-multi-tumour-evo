...
#SBATCH --output=data/3_cellphy_log/single/%j_single.out
...
process_single_folder() {
    local iteration_identifier="_pop99089_sites10212r3"
    local iteration_folder="$single_ccoal_output_folder/_pop99089_sites10212r3"
    ...
    cellphy.sh RAXML --msa "$haplotype_file" --model GT16+FO+E --seed 2 --threads 2 --force perf_threads --prefix "$single_output_prefix" --redo
}

execute_r_script() {
    local iteration_identifier="_pop99089_sites10212r3"
    local iteration_folder="$single_ccoal_output_folder/_pop99089_sites10212r3"
    local gene_tree_file=$(find "$simphy_folder/_pop99089_sites10212r3" -type f -name 'g_trees*.trees' -print -quit)
...
log_file="data/3_cellphy_log/single/master_log.txt"
echo "Job started at $(date)" >> "$log_file"

mv "data/3_cellphy_log/single/${SLURM_JOB_ID}_single.out" "data/3_cellphy_log/single/_pop99089_sites10212r3_${SLURM_JOB_ID}_single.out"
