...
for (iteration_identifier in iteration_identifiers) {
  if (any(grepl("^full_hap_consensus_0.36.fasta", list.files(file.path("data/2_ccoal", iteration_identifier, "full_haplotypes_dir"))))) {
...
cutoff_sh <- readLines("scripts/cheat_folder/ncount/cutoff/cutoff.sh")
...
writeLines(modified_cutoff_script, "scripts/cheat_folder/ncount/cutoff/mod_cutoff.sh")

cmd_cutoff <- paste("sbatch --export=iteration_identifier=\"", iteration_identifier, "\" scripts/cheat_folder/ncount/cutoff/mod_cutoff.sh", sep="")
