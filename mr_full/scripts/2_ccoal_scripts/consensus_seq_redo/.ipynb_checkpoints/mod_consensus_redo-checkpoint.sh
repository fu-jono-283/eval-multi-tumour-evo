...
#SBATCH --output=2_ccoal_slurmout/%j_con.out
#SBATCH --error=2_ccoal_slurmout/%j_con.err
#SBATCH -A uoa03626    
#SBATCH --ntasks 1       
...
output_parent_folder="2_ccoal_folder"

iteration_folder="$output_parent_folder/$exported_iteration_identifier"

update_consensus_sequence() {
    full_hap_file="$1"
    cmd_python="python cellcoal_scripts/consensus_cutoff.py \"$full_hap_file\""
