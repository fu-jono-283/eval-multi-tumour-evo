...
for (iteration_folder in iteration_folders) {
  ...
  if (length(cutoff_counts) > 0) {
    ...
    count_data <- rbind(count_data, row)
  write.csv(count_data, file = "dataframes/count_data", row.names = FALSE)
  }
}

existing_data <- read.csv("dataframes/part1_template/combined_data.csv")
...
write.csv(mega_data, file = "dataframes/zexp/allcountdata.csv", row.names = FALSE)
