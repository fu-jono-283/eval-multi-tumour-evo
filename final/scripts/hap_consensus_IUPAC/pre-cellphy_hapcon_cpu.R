...
for (iteration_identifier in iteration_identifiers) {
  if (any(grepl("^hapcon_IUPAC.raxml.bestTree", list.files(file.path("data/2_ccoal", iteration_identifier))))) {
    cat("Skipping iteration:", iteration_identifier, " - folder contains hapcon_IUPAC.raxml files\n")
    next
  }
...
cellphy_hapcon_sh <- readLines("scripts/3_cellphy_scripts/hap_consensus_v2/cellphy_hapcon.sh")
...
writeLines(modified_hapcon_script, "scripts/3_cellphy_scripts/hap_consensus_v2/mod_cellphy_hapcon.sh")

cmd_cellphy_hapcon <- paste("sbatch scripts/3_cellphy_scripts/hap_consensus_v2/mod_cellphy_hapcon.sh", sep="")
