...
for (iteration_identifier in iteration_identifiers) {
     if (any(grepl("vcf.raxml.log", list.files(file.path("data/2_ccoal", iteration_identifier))))) {
...
cellphy_vcf_sl <- readLines("scripts/3_cellphy_scripts/vcf_consensus/vcf_consensus.sh")
...
writeLines(modified_vcf_script, "scripts/3_cellphy_scripts/vcf_consensus/mod_vcf_consensus.sh")

cmd_cellphy_vcf <- paste("sbatch scripts/3_cellphy_scripts/vcf_consensus/mod_vcf_consensus.sh", sep="")
